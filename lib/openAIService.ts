...
if (!OPENAI_API_KEY) {
  console.warn('[openAIService] OPENAI_API_KEY not set; OpenAI provider will be unavailable.');
}
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (error?.status === 429 || error?.message?.includes('429')) {
        ...
        console.log(`[openai retryWithBackoff] Rate limited, waiting ${waitTime}ms before retry ${attempt + 1}/${maxRetries}`);
